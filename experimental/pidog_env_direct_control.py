#!/usr/bin/env python3
"""EXPERIMENTAL / DEPRECATED — direct-joint-control PiDog env (27-dim obs). DO NOT TRAIN ON THIS.

This env asks PPO to discover a gait from scratch: direct joint targets, no scripted base gait,
no gait-clock. It does not work. On 2026-07-09 eight PPO configurations (~3.5 h CPU) were run
against it -- reward reshaping, ent_coef sweeps, gamma/termination tuning, curriculum resume from
a balance-mastered policy, action-std re-inflation, and a velocity-target reward. EVERY run
converged either to standing still (~15 mm forward over 1000 steps) or to a forward dive that
falls by step ~35. None came close to the 200 mm bar.

The canonical env is ../pidog_env.py: 29-dim obs, policy emits a SMALL RESIDUAL on top of a
known-symmetric scripted diagonal trot (+2 gait-clock dims). Its own comment: "Base gait is
correct by construction ... there is NO 'discover-a-gait' problem." It produces policies that
pass eval_mvp.py (run15: 432 mm forward, 1000 steps).

pidog.xml is NOT at fault and needs no changes -- the working env uses the byte-identical model.

Kept only as a record of the negative result. Nothing imports this module.
"""
